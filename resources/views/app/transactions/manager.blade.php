...
@section('title', auth()->user()->name.' - সকল ধরনের লেনদেন')
...
@section('content')
  <div class="container-fluid">
    <h4 class="text-success text-center fw-bolder">সকল ধরনের লেনদেন - {{ auth()->user()->name }}</h4>
    <hr>
    <form id="filter-form">
      <div class="row">
        <div class="col-md-3">
  ...
  <script>
    ...
    $(document).ready(function () {
      ...
      $('#transactions-table').DataTable({
        ...
        "order": [[0, 'desc']],
        "ajax": {
          "url": "{{ route('data.manager.transactions') }}",
          data: function (d) {
            d.date_filter = $('#date-filter').val();
            d.account_no_filter = $('#account-no-filter').val();
            d.transactionable_type_filter = $('#transactionable-type-filter').val();
