                    ...
                    <div class="card-body">
                        <div class="user-avatar-section">
                            <div class="d-flex align-items-center flex-column">
                                ...
                                <div class="user-info text-center">
                                    <h4>
                                        <a href="{{ route('users.show',$takenLoan->user_id) }}">{{ $takenLoan->user->name }}</a>
                                    </h4>
                                    <div class="mb-2">মোবাইল নং- <span class="text-success">{{ $takenLoan->user->phone1 }}</span></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="info-container">
                            <table class="table table-sm table-bordered">
                                <tr>
                                    <td class="fw-bolder px-1">হিসাব নং:</td>
                                    <td class="px-1">{{ $takenLoan->account_no??'' }}</td>
                                </tr>
                                <tr>
                                    <td class="fw-bolder px-1">ঋণের পরিমাণ:</td>
                                    <td class="px-1">{{ $takenLoan->loan_amount??'' }}</td>
                                </tr>
                                <tr>
                                    <td class="fw-bolder px-1">সুদের হার:</td>
                                    <td class="px-1">{{ $takenLoan->interest1??'' }}% {{ $takenLoan->interest2>0?' | '.$takenLoan->interest2.'%':'' }}</td>
                                </tr>
                                <tr>
                                    <td class="fw-bolder px-1">পরিমাণ পর্যন্ত:</td>
                                    <td class="px-1">{{ $takenLoan->upto_amount??'N/A' }}</td>
                                </tr>
                                <tr>
                                  <td class="fw-bolder px-1">তারিখ</td>
                                  <td class="px-1">তাঃ {{ date('d/m/Y',strtotime($takenLoan->date))}} <br>
                                    আঃ {{ date('d/m/Y',strtotime($takenLoan->commencement)) }}
                                  </td>
                                </tr>

                                <tr>
                                    <td class="fw-bolder px-1">ঋণ ফেরত:</td>
                                    <td class="px-1">{{ $takenLoan->loan_amount - $takenLoan->remain }}</td>
                                </tr>

                                <tr>
                                    <td class="fw-bolder px-1">অবশিষ্ট ঋণ:</td>
                                    <td class="px-1">{{ $takenLoan->remain }}</td>
                                </tr>


                                <tr>
                                    <td class="fw-bolder px-1">নোট:</td>
                                    <td class="px-1">{{ $takenLoan->note }}</td>
                                </tr>

                            </table>

                          @if(isset($takenLoan->guarantor))

                            <h4 class="mt-3 mb-0">জামিনদার</h4>
                            <table class="table table-sm table-bordered">
                              <tr>
                                <th class="px-1 py-1 fw-bolder">নাম</th> <td class="px-1 py-1">
                                  @if($takenLoan->guarantor->user_id)
                                    <a href="/users/{{$takenLoan->guarantor->user_id}}" target="_blank">{{ $takenLoan->guarantor->name }}</a>
                                  @else
                                    {{ $takenLoan->guarantor->name }}
                                  @endif
                                  </td>
                              </tr>
                              <tr>
                                <th class="px-1 py-1 fw-bolder">মোবাইল</th> <td class="px-1 py-1">{{ $takenLoan->guarantor->phone }}</td>
                              </tr>
                              <tr>
                                <th class="px-1 py-1 fw-bolder">ঠিকানা</th> <td class="px-1 py-1">{{ $takenLoan->guarantor->address }}</td>
                              </tr>
                            </table>
                          @endif
                        </div>
              ...
              <div class="card">
                <table class="loan-transactions table-bordered table table-sm">
                  ...
                  <tr>
                    ...
                    <th class="fw-bolder">সুদের পরিমাণ</th>
                    <th class="fw-bolder"># কিস্তি সংখ্যা</th>
                    <th class="fw-bolder">মোট সুদ</th>
                  </tr>
                  </thead>
                </table>
              </div>
    ...
    <script>
        ...
        function loadLoanCollection(loan_id ='')
        {
            $('.loan-transactions').DataTable({
                ...
                "ordering": false,
                "ajax":{
                    "url": "{{ url('dataTakenLoanTransaction') }}",
                    type: "GET",
                    data: {loanId:loan_id}
                },
                "columns": [
