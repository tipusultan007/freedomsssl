...
class FdrProfitController extends Controller
{
    ...
    public function allFdrProfits(Request $request)
    {
        ...
        {
            $posts = FdrProfit::join('users','users.id','=','fdr_profits.user_id')
                ->join('users as creator', 'creator.id', '=', 'fdr_profits.created_by')
                ->select('fdr_profits.*','users.name as name','users.phone1','creator.name as created_by')
                ->offset($start)
                ->limit($limit)
                ->orderBy('id','desc')
                ->get();
            ...
            $posts =  FdrProfit::join('users','users.id','=','fdr_profits.user_id')
                ->join('users as creator', 'creator.id', '=', 'fdr_profits.created_by')
                ->select('fdr_profits.*','users.name as name','users.phone1','creator.name as created_by')
                ->where('fdr_profits.account_no','LIKE',"%{$search}%")
            ...
            $totalFiltered =  FdrProfit::join('users','users.id','=','fdr_profits.user_id')
                ->join('users as creator', 'creator.id', '=', 'fdr_profits.created_by')
                ->select('fdr_profits.*','users.name as name','creator.name as created_by')
                ->where('fdr_profits.account_no','LIKE',"%{$search}%")
                ->orWhere('users.name', 'LIKE',"%{$search}%")
                ->count();
        }
    ...
    public function store(Request $request)
    {
        ...
        $cashout = Cashout::create([
            'cashout_category_id' => 6,
            'account_no' => $fdr->account_no,
            'fdr_profit_id' => $fdrProfit->id,
            'amount' => $fdrProfit->profit - $fdrProfit->other_fee + $fdrProfit->grace,
            'date' => $fdrProfit->date,
            'trx_id' => $fdrProfit->trx_id,
            'created_by' => $fdrProfit->created_by,
            'user_id' => $fdrProfit->user_id,
        ]);
    ...
    public function destroy($id)
    {
        ...
        {
            /*$depositAccount = Account::find(9); //EXPENSE (PAID PROFIT+)
            $depositAccount->balance -= $profit->total;
            $depositAccount->save();

            $cashAccount = Account::find(4); //ASSET (CASH-)
            $cashAccount->balance += $profit->total;
            $cashAccount->save();*/
            //PaidProfitAccount::delete($profit->trx_id)
        ...
        PaidProfitAccount::delete($fdrProfit->trx_id,'fdr');
        //Transaction::where('trx_id',$fdrProfit->trx_id)->delete();

        Cashout::where('fdr_profit_id',$id)->delete();
        $fdrProfit->delete();
