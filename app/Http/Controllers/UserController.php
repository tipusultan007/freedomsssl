...
class UserController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('app.users.create', compact('roles', 'breadcrumbs'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $id               = $lastId->id + 1;
        $data             = $request->except(['national_id', 'birth_id', 'profile_photo_path']);
        ...
        if ($request->hasFile('national_id')) {
            ...
            $new_name = "nid_" . $id . "_" . time() . '.' . $image->getClientOriginalExtension();
        ...
        if ($request->hasFile('birth_id')) {
            ...
            $new_name = "birthid_" . $id . "_" . time() . '.' . $image->getClientOriginalExtension();
            $image->move(public_path('images'), $new_name);
        ...
        if ($request->hasFile('profile_photo_path')) {
            ...
            $new_name = "profile_" . $id . "_" . time() . '.' . $image->getClientOriginalExtension();
            $image->move(public_path('images'), $new_name);
            $user->profile_photo_path = $new_name;
            $user->save();
        }
        ...
        switch ($account_type) {
            case 'daily_savings':
                ...
                $nominee = Nominees::create([
                    ...
                    'user_id' => $request->nominee_user_id??''
                ]);
                break;
            case 'dps':
                $data['user_id'] = $user->id;
    ...
    {
        $user              = User::find($id);
        $dps               = Dps::where('user_id', $id)->count();
        $daily_savings     = DailySavings::where('user_id', $id)->count();
        $special_dps       = SpecialDps::where('user_id', $id)->count();
        $daily_loans       = DailyLoan::where('user_id', $id)->count();
        $dps_loans         = DpsLoan::where('user_id', $id)->count();
        $special_dps_loans = SpecialDpsLoan::where('user_id', $id)->count();
