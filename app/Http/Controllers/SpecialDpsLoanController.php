...
class SpecialDpsLoanController extends Controller
{
    ...
    public function dataSpecialDpsLoans(Request $request)
    {
        ...
        if (empty($request->input('search.value'))) {
            $posts = SpecialDpsLoan::leftJoin('users', 'users.id', '=', 'special_dps_loans.user_id')
                ->leftJoin('users as c', 'c.id', '=', 'special_dps_loans.created_by')
                ->select('special_dps_loans.*', 'users.name as name', 'users.phone1', 'users.profile_photo_path', 'c.name as createdBy')
                ->offset($start)
                ->limit($limit)
            ...
            $posts = SpecialDpsLoan::leftJoin('users', 'users.id', '=', 'special_dps_loans.user_id')
                ->leftJoin('users as c', 'c.id', '=', 'special_dps_loans.created_by')
                ->select('special_dps_loans.*', 'users.name as name', 'users.phone1', 'users.profile_photo_path', 'c.name as createdBy')
                ->where('special_dps_loans.account_no', 'LIKE', "%{$search}%")
            ...
            $totalFiltered = SpecialDpsLoan::leftJoin('users', 'users.id', '=', 'special_dps_loans.user_id')
                ->leftJoin('users as c', 'c.id', '=', 'special_dps_loans.created_by')
                ->where('special_dps_loans.account_no', 'LIKE', "%{$search}%")
                ->orWhere('users.name', 'LIKE', "%{$search}%")
                ->count();
    ...
    public function store(Request $request)
    {
        ...
        if ($loan) {
            ...
            $cashout = Cashout::create([
                'cashout_category_id' => 8,
                'account_no' => $takenLoan->account_no,
                'special_loan_id' => $takenLoan->id,
    ...
    public function destroy($id)
    {
        ...
        foreach ($takenLoans as $takenLoan) {
            ...
            Cashout::where('special_loan_id', $takenLoan->id)->delete();
            SpecialLoanAccount::delete($takenLoan->trx_id);
            $takenLoan->delete();
        }
        ...
        foreach ($dpsInstallments as $dpsInstallment) {
            ...
            SpecialLoanInterest::where('special_installment_id', $dpsInstallment->id)->delete();
            SpecialLoanCollection::where('special_installment_id', $dpsInstallment->id)->delete();
