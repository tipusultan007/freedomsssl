...
class OtherFeeAccount extends Controller
{
    public static function create($data)
    {
        $account = Account::find(24);
        $account->balance += $data['other_fee'];
        $account->save();
    ...
    {
        $transaction = Transaction::where('account_id',24)->where('trx_id',$trxId)->first();
        if ($transaction) {
            $account = Account::find(24);
            $account->balance -= $transaction->amount;
            $account->save();
