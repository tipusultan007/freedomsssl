...
class LateFeeAccount extends Controller
{
    public static function create($data)
    {
        $account = Account::find(22);
        $account->balance += $data['late_fee'];
        $account->save();
    ...
    {
        $transaction = Transaction::where('account_id',22)->where('trx_id',$trxId)->first();
        if ($transaction) {
            $account = Account::find(22);
            $account->balance -= $transaction->amount;
            $account->save();
