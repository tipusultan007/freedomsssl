...
class SpecialLoanPaymentAccount extends Controller
{
    public static function create($data)
    {
        $account = Account::find(29);
        $account->balance += $data['loan_installment'];
        $account->save();
        $account = Account::find(28);
        $account->balance -= $data['loan_installment'];
        $account->save();
    ...
    {
        $transaction = Transaction::where('account_id',29)->where('trx_id',$trxId)->first();
        if ($transaction) {
            $account = Account::find(29);
            $account->balance -= $transaction->amount;
            $account->save();
