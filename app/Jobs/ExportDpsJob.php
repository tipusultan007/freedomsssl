...
class ExportDpsJob implements ShouldQueue
{
  ...
  public function handle(): void
  {
    ...
    $filename = 'মাসিক_সঞ্চয়_তালিকা_রিপোর্ট_' . now()->format('d-m-Y_H-i-s') . '.csv';
    ...
    try {
      ...
      $csv->setOutputBOM(Reader::BOM_UTF8); // Set UTF-8 BOM (Byte Order Mark)
      ...
      Dps::with('user','dpsPackage')->chunk($chunkSize, function ($dailySavings) use ($csv, $columnOrder) {
        foreach ($dailySavings as $dailySaving) {
          ...
          $mappedData = array_map(function ($column) use ($dailySaving) {
            if ($column === 'name') {
              ...
              return $dailySaving->dpsPackage->name;
            }
